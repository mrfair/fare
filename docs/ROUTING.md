...
- `requiresAuth = true` จะเรียก `isAuthed()` (ดู `src/app/auth.ts:1-54`) ซึ่งอ่าน session mirror จาก `localStorage` (`spa_demo_session_v1`) เพื่อให้ guard ทำงานแบบ synchronous; ถ้า `false` จะ redirect ไป `/login?next=…`
- `roles = ["admin"]` ระบุชุดบทบาทที่ผู้ใช้ต้องมี (router เรียก `getRoles()` ที่ดึงมาจาก session ที่ mirror โดย `bootstrapSession()` ก่อน router start `src/app/main.ts:1-20`)
...
- ฟังก์ชัน `bootstrapSession()` (`src/app/auth.ts:20-55`) จะเรียก `/api/auth/me` (credentials include) เพื่อตรวจสอบ cookie ที่ backend เซ็ตไว้แล้ว mirror ลง `localStorage`; เรียกครั้งแรกใน `main.ts` ก่อน router start (`src/app/main.ts:1-20`) เพื่อ ensure กรณี login เสร็จแล้ว
...
- ถ้าอยากออกจากระบบก็เรียก `logout()` (`src/app/auth.ts:40-58`), ซึ่ง POST ไป `/api/auth/logout` แล้วล้าง session mirror ด้วย `setSession(null)`
...
```
- `ctx` มาจาก router (ดู `src/app/router.ts:170-230`) และรวม `isAuthed`, `roles`, `url`, `navigate` ฯลฯ
